---
description: One-shot scan for CLAUDE.md §1 Critical Rule violations across the repo
---
...
2. **Rule 2 — no JavaScript.** Glob for `**/*.{js,jsx}` excluding `node_modules`, `.turbo`, `dist`, `build`, hook scripts under `.claude/hooks/` (those are intentionally ESM), and config files at the repo root (`.eslintrc.js`, `*.config.js`).
...
6. **Rule 8 — no `console.log` in prod.** Grep `console.(log|error|warn|info|debug)` across `apps/**`, `packages/**`, `services/**`. Exclude `*.test.ts*`, `test-utils/`, `.claude/hooks/`, and Edge Function `catch` blocks (the §11 template explicitly allows one `console.error` in the final catch).
